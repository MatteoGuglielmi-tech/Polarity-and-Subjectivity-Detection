...
\begin{itemize}
    ...
    \item concerning the \textit{dataset split} dimensions:
        ...
        \item the IMDB dataset has been splitted following the built-in format of the dataset itself (\numprint{25000} samples for training
            and \numprint{25000} samples for evaluation)
        \end{itemize}
...
In \textbf{\Cref{fig:singlerunacc}} and \textbf{\Cref{fig:singlerunloss}} respectively, the training and 
evaluation accuracies and losses on a single run for the BiLSTM model (discussed in \textbf{\Cref{subsec:custom}}) are shown. \\
